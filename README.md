...
| `searchCategoryCount` | `20` | How deep the drill-in fetches each category. Larger than `searchCount`, which is tuned for a merged search showing three rows a service: the rows behind **More ›** must already be in hand, since expanding is a local reveal rather than another round trip. Minimum 1. |
...
| `more` | `More` — on the right of a service heading, beside the chevron, saying what pressing the heading does |
