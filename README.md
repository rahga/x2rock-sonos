...
| `searchCount` | `20`, or `5` merged | Hits fetched **per service**, not in total. Minimum 1. Twenty is right for one service and far too many across thirty-five, so the default drops when `searchService` is `"all"` or `"linked"`. |
| `searchCategory` | `""` | Which of the service's categories the search row queries, passed to the CLI as `-c`. Empty means the CLI's default: the service's `all` when it has one, else its first category. In a merged search it also *narrows the field* — a service with no category by that name is skipped rather than searched in the wrong one, so `"albums"` asks only services that really have albums. Worth setting for a library-shaped service — Plex has no `all` and leads with `artists`, so a picker pointed at Plex wants `"tracks"` here. `x2rock search -s <service>` with no term lists what a service offers. |
